...
class DoplnkyContentProvider(object):
	...
	def root(self):
		if 'search' in self.provider.capabilities():
			...
			doplnky.add_dir(_("Search"), params, util.icon('search.png'))
		...
		if streams is not None:
			if type(streams) == type([]):
				for stream in streams:
					doplnky.add_play('%s - %s[%s]' % (params['title'], stream['title'], stream['quality']), stream['url'], subs=stream['subs'])
			else:
				#ulozto,bezvadata..
				doplnky.add_play("%s" % params['title'], streams['url'], subs=streams['subs'])
	...
	def list(self, items):
		...
		for item in items:
			if item['type'] == 'dir':
				...
				doplnky.add_dir(_("Next"), params, util.icon('next.png'))
			elif item['type'] == 'prev':
				params.update({'list':item['url']})
				doplnky.add_dir(_("Previous"), params, util.icon('prev.png'))
			elif item['type'] == 'new':
				params.update({'list':item['url']})
				doplnky.add_dir(_("New"), params, util.icon('new.png'))
			elif item['type'] == 'top':
				params.update({'list':item['url']})
				doplnky.add_dir(_("Top"), params, util.icon('top.png'))
	...
	def render_dir(self, item):
		...
		doplnky.add_dir(title, params, img, infoLabels=self._extract_infolabels(item))
...
class DoplnkyLoginRequiredContentProvider(DoplnkyContentProvider):
	...
	def root(self):
		if not self.provider.login():
			raise CustomInfoError(_("Cannot login, not correct login data"))
		else:
